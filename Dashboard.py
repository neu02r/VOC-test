...
if __name__ == '__main__':
    ...
    with tab4:
        with st.expander('17W 이슈 및 조치사항', expanded=True):
            ...
            st.dataframe(temp1)
            st.write("택배 기사님 오배송으로 인해 문의글로 인입이되어, 배송기사님과 먼저 통화 -> 기사님께서 본인의 명확한 실수이기 때문에 고객님과 직접 통화를 원하신다고 하여, 인입시 고객님께 즉시 기사님 번호 전달 (선사과를 하지 않은 부분에 대해서 실무자분과 피드백 진행)")
            st.write('\n\n')
            ...
            st.dataframe(temp3)
            st.write("4.26(금) 부분 교환 및 커피 쿠폰을 말씀드렸으나,  1세트에 대해 반품원하셔서 진행 -> 물류팀측으로 전달하여 경북 영주시 CJ영업소와 통화하여 재발되지 않도록 요청 완료 ")
            st.write('\n\n') 
